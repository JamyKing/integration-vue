	...
	<view class="post-list">
		<load-refresh ref="loadRefresh" :backgroundCover="'#F3F5F5'" :pageNo="currPage" :totalPageNo="totalPage" @loadMore="loadData(10)" @refresh="loadData(20)">
			<view slot="content-list">
				<uni-swipe-action>
				    <uni-swipe-action-item v-for="(item,index) in list" :key="index" :options="options" @click="delFun(item.id)" class="swipe-action-item">
						<view @click="detail(item.id)" class="post-item animated fadeIn" :class="{'u-f':item.type < 3}">
							<!-- 左图右文 -->
							...
							<block v-if="item.type === 3">
								<view class="img-only" :style="`background-image: url(${item.blogPics[0].pic});`">
									...
									<image src="../../static/imgs/shutter.png" class="shutter-three"></image>
								</view>
							</block>
	...
	export default {
		...
		methods: {
			async loadData(val) {
				...
				that.$api.request('/app/my/getMyBlogList', 'POST', {
			...
			delFun(id) {
				...
				uni.showModal({
					...
					success: (e) => {
						if (e.confirm) {
							that.$api.request('/app/blog/deleteBlog', 'GET', {
								blogId: id
							}, failres => {
								that.$api.msg(failres.msg)
							}).then(res => {
